...
function SmallChangeVal(val) {
	var new_val = Math.floor(10*Math.random());
...
//
function SparklineTestData(base, delta){
	var arr = [];
	arr.push([0])
	for (var i=2; i<9; i++){
		arr.push([Math.floor(base - (delta/2) + 0 * Math.random())])
	}
...
function SparklineLoop(){
	...
	SparkLineDrawBarGraph($('#sparkline-4'), sparkline_arr_4.map(SmallChangeVal), 'green');
}
//
